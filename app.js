...
function _applyHomeVoice(rawTxt){
    ...
    if(isBuyDoll||isSellDoll){
        ...
        setTimeout(()=>{
            ...
            toast(`💲 ${isBuyDoll?'شراء':'بيع'} دولار — ${customer}: ${amount.toLocaleString('ar-DZ')}$ × ${rate.toLocaleString('ar-DZ')} = ${fmt(amount*rate,0)} دج | اضغط حفظ للتأكيد`,'info');
        },420);
        return;
    }
...
function upd(){
    ...
    if(_gst){
        const _tbW=(typeof _tarbahTotalW==='function')?_tarbahTotalW():0;
        const _total730=_bk.raw_730 + _bk.raw_24*(1000/730) + _tbW;   /* + ذهب الترباح */
        _gst.textContent=fmt(_total730,2)+' غ';
        const _sub=document.getElementById('goldSaleSub');
        if(_sub)_sub.textContent=_tbW>0?('عيار 730 · منها ترباح '+fmt(_tbW,2)+' غ'):'عيار 730';
    }
    const _nv=net();
...
window.openGiveTake=(t)=>{
    ...
    document.getElementById('gtTitle').textContent=(t==='give'?'🟢 تسليم (أعطيت)':'🔴 استلام (قبضت)')+' • v107';
...
window.saveGT=()=>{
    ...
    if(gtType==='give'){
        ...
        if(isG730||isG24){
            /* حارس صارم: الوزن الخام يجب أن يكون موجوداً فعلاً في الكوفر */
            const _pool=isG730?'730':'24';
            const _bars=isG730?g730:g24;
            const _avail=_bars.reduce((s,b)=>s+(Number(b.w)||0),0);
            if(_avail<a-0.001){
                return toast(`🚫 لا توجد سبائك كافية في الكوفر (متاح: ${fmt(_avail,2)} غ · مطلوب: ${fmt(a,2)} غ)`,'error');
            }
            const r=_pickBarsToRemove(_pool,a);
            /* تحقّق أن الاقتطاع غطّى الوزن كاملاً */
            const _covered=(r.barsRemove||[]).reduce((s,id)=>{const b=_bars.find(x=>x.id===id);return s+(b?Number(b.w)||0:0);},0)
                          +(r.barUpdates||[]).reduce((s,u)=>{const b=_bars.find(x=>x.id===u.id);return s+(b?(Number(b.w)||0)-(Number(u.newW)||0):0);},0);
            if(_covered<a-0.01){
                return toast('🚫 تعذّر تغطية الوزن من سبائك الكوفر — راجع المخزون','error');
            }
...
/* ═══════════ ترباح — ملاحظات حرّة (وزنها يُضاف لعرض «ذهب البيع» فقط) ═══════════ */
/* مجموع أوزان الترباح (غرامات) — للعرض في بطاقة ذهب البيع */
window._tarbahTotalW=()=>{
    try{
        const num=v=>{const n=parseFloat(String(v||'').replace(/\s/g,'').replace(',','.'));return isFinite(n)?n:0;};
        return (window._tarbahList||[]).reduce((s,x)=>s+Math.max(0,num(x.weight)),0);
    }catch(e){return 0;}
};
...
window.doXfer=()=>{
    ...
    /* التحويل لنفس الزبون مقبول بلا اعتراض (خاصّةً تحويل العيار: 730⇄24) */
    const W=readNum('xferW');
...
/* ═══════════ إغلاق أي نافذة منبثقة بلمس الخلفية (عام لكل النوافذ) ═══════════ */
(function _bindOverlayDismiss(){
    const _close=(ov)=>{
        ...
        }catch(e){ try{ov.classList.remove('active');}catch(_){} }
    };
    document.addEventListener('pointerdown',(e)=>{
        /* الخلفية فقط: العنصر الملموس هو الغلاف نفسه لا محتواه */
        const ov=e.target;
        if(!ov||!ov.classList)return;
        if(ov.classList.contains('modal-overlay')&&ov.classList.contains('active')){
            /* نوافذ حسّاسة لا تُغلق باللمس العَرَضي */
            if(ov.id==='loginOverlay'||ov.id==='serialOverlay')return;
            _close(ov);
        }
    },true);
    /* زر الرجوع في الأندرويد يغلق آخر نافذة مفتوحة */
